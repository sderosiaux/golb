---
title: A lot of performance tips
...
Some of the items may be irrelevant nowadays because it was optimized since.{.info}
...
svgs are being preferred to custom fonts. They are more targeted and very lightweight.{.info}
...
Mobiles do not have the same capability than desktops, and the performances could be worse.{.warn}
...
Note: `translate: transform` transition is using *subpixel* rendering instead of pixel per pixel rendering as with `top`/`left` transitions: a smoother rendering.{.info}
