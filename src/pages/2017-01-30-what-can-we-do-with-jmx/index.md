...
category: 'JVM'
description: "JMX is an API used to expose data from any Java program to the outside. It's used by tons of softwares and libraries because you can then gather the data in any other system."
---
...
object JMX extends App {
  val server = ManagementFactory.getPlatformMBeanServer()
  server.registerMBean(new Metric(1.0), ObjectName.getInstance("com.sderosiaux:type=Metric"))
  server.registerMBean(new Metric(2.0), ObjectName.getInstance("com.sderosiaux:type=Metric,subtype=Sub"))
...
mainClass in (Compile, run) := Some("com.sderosiaux.JMXTest")
```
...
object WithCamel extends App with LazyLogging {
  ...
  server.registerMBean(metric, ObjectName.getInstance("com.sderosiaux:type=MyMetric"))
  ...
  context.addRouteDefinition(new RouteDefinition(
    ...
    .bean(metric, "update"))
  context.addRouteDefinition(new RouteDefinition(
    "jmx:platform?objectDomain=com.sderosiaux&key.type=MyMetric")
    .to("log:com.sderosiaux:INFO"))
...
Route: route2 started and consuming from: jmx://platform?key.type=MyMetric&objectDomain=com.sderosiaux&observedAttribute=Value
Total 2 routes, of which 2 are started.
Apache Camel 2.18.1 (CamelContext: camel-1) started in 0.647 seconds

INFO com.sderosiaux:INFO - Exchange[ExchangePattern: InOnly, BodyType: String, Body:
<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<AttributeChangeNotification xmlns="urn:org.apache.camel.component:jmx">
  <source>com.sderosiaux:type=MyMetric</source>
...
```scala
context.addRouteDefinition(new RouteDefinition("direct:testing").to("log:com.sderosiaux:INFO"))
```
...
INFO com.sderosiaux:INFO - Exchange[ExchangePattern: InOnly, BodyType: String, Body: hello there!]
```
...
- `-q`: if we want to specify some quartz properties. JMXTrans has some defaults in the file `quartz-server.properties`. Quartz has [tons of options](http://www.quartz-scheduler.org/documentation/quartz-2.2.x/configuration/) such as its threadpool config, listeners, plugins, misc thresholds.
...
server.registerMBean(metrics, ObjectName.getInstance("com.sderosiaux:type=RandomMetrics"))
```
...
"queries": [{
  ...
  "obj": "com.sderosiaux:type=RandomMetrics",
  "attr": []
}],
```
...
mainClass in (Compile, run) := Some("com.sderosiaux.JMXTest") // a simple blocking app
```
...
```shell
echo "com.sderosiaux.test 42 $(date +%s)" | nc 192.168.0.11 2003
```
...
T 172.17.0.1:54598 -> 172.17.0.2:2003 [AP]
  com.sderosiaux.test 42 1486316847.
########
T 172.17.0.1:54600 -> 172.17.0.2:2003 [AP]
  com.sderosiaux.test 43 1486316862.
####
```
