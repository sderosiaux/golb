...
CQRS is a good pattern to reason about a whole system, when we are used to work with it. There is definitely a **steep learning curve** to apprehend the separations and the constraints. Instead of thinking *globally*, we must think *locally* (`Commands`, `Events`). It's **difficult to predict the general behavior**. The features are broken into pieces and we can have different actors and different timelines to complete them (and more case of failures). The eventual-consistency can also add its bag of issues we didn't have before with a monolithic design. We trade control over scalability and flexibility (CQRS).

CQRS is generally coupled to a good DDD thinking beforehands, to avoid mixing different domains (or the inverse, split domains we shouldn't), or creating weird Aggregates that don't fit the business after all. The distinction between Write and Read models help tremendously.
...
About the Read model, it's totally *YOLO*, *FFA*, because it does not matter, it has no impact. It is meant to evolve a lot, following of the front-end or consumers needs. I just want to **protect the Write Model**.

CQRS is also meant to work with Events. They are about the business. They offer traceability and decouple systems among other things. The Event Sourcing approach is useful and very safe by nature but comes with its own bagages of constraints (performance, snapshots, storage..). Working with events means we have an **Event-Driven system**. As such, a feature can required 20 hops through the message bus, to complete (a succession of boundaries Event Handlers / Command Handlers). Without a good **performance tuning** and a good **observability** of the system (logs/metrics/tracing, and a good understanding of how things correlate together), it can perform poorly. Even if it can scale, it won't be efficient.
