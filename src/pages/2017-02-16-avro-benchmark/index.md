---
title: "Serializing data efficiently with Apache Avro and dealing with a Schema Registry"
date: "2017-03-02T02:08Z"
is_blog: true
path: "/articles/2017/03/02/serializing-data-efficiently-with-apache-avro-and-dealing-with-a-schema-registry/"
language: "en"
...
[[warn]]
|A same writer/reader can be used as many times as we want, as long as the data fits its schema.
...
[[info]]
|Note that Strings are slower to handle than Doubles for instance. 80x-Doubles records offer ~20% more throughput than 80x-Strings records.
...
[[info]]
|schema-repo is not necessarily specialized for Avro Schemas. The "schemas" (which are simply the value of the key `(subject, version)`) can actually be anything we want (integers, strings, whatever), using Avro Schemas is only one of their usage.
...
[[warn]]
|Note that there is now way to remove a schema from a subject, it's immutable. It's only possible by removing them manually from their storage.
...
[[info]]
|It's incompatible because we removed a field: the old schemas are not compatible anymore. If an app was still reading events using the original schema, it wouldn't be able to deserialize the new events because it expects a `first` property (no default value).
...
[[warn]]
|All the POST/PUT must send the header: `Content-Type: application/vnd.schemaregistry.v1+json` to be taken into account.
...
[[info]]
|We should pass a subject and register the schema here instead of directly passing a schemaId, to save the new schemas transparently.
